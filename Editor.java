...
public class Editor extends JFrame {	
	...
	public void drawSketch(Graphics g) {
		// TODO: YOUR CODE HERE
		int[] idList = sketch.getSortedIds().stream().mapToInt(Integer::intValue).toArray();
		for(int i=0;i<idList.length;i++){
			sketch.getShape(idList[i]).draw(g);
		}
		if(curr!=null){
			curr.draw(g);
		}
	...
	private void handlePress(Point p) {
		...
		if(mode==Mode.DRAW){
			...
			if(shapeType.equals("ellipse")) {
				...
				Polyline currP = new Polyline(color);
				currP.addToPointList((int)drawFrom.getX(),(int)drawFrom.getY());
				curr = currP;
			}
			repaint();
		}
			...
			int id = sketch.getIdOfTopShape(p);
			if(id != -1){
				moveFrom = p;
				movingId = id;
			} else {
	...
	private void handleDrag(Point p) {
		...
		if(mode==Mode.DRAW && drawFrom!=null){
			if(shapeType.equals("ellipse")) {
				...
				((Polyline)curr).addToPointList((int)p.getX(), (int)p.getY());
			}
		}
